...
/**
 * Fancy image-based progress bar class (supports vertical and horizontal progress indication)
 * @author benjamin
 *
 */
public class Meter extends Sprite {
	...
	/**
	 * Default constructor
	 * @param position of top left corner
	 * @param width of progress bar
	 * @param height of progress bar
	 * @param scWidth screen width
	 * @param scHeight screen height
	 * @param vertProgress should the progress be indicated vertically
	 * @param texFG foreground texture
	 * @param texBG background texture
	 * @param progress intial progress
	 */
	public Meter(Vector2 position, float width, float height,
			float scWidth, float scHeight, boolean vertProgress, Texture texFG, Texture texBG, float progress) {
		super();
	...
	public void draw(SpriteBatch spriteBatch, float delta) {
		...
		if (this.vertProgress){
			setRegion(0, (int)(fg.getHeight() - this.progress*fg.getHeight()), fg.getWidth(), (int)(this.progress*fg.getHeight()));
